...
def pad(img, keypoint2d, size):
    w, h = img.size 
    pad_left = pad_right = pad_top = pad_bottom = 0
    if size-w > 0:   
        pad_left = math.floor((size - w) / 2)
        pad_right = math.ceil((size - w) / 2)  
    if size-h > 0: 
        pad_top = math.floor((size - h) / 2)
        pad_bottom = math.ceil((size - h) / 2)
    img = np.asarray(img)
...
class Pad(object):
    ...
    def __call__(self, img, keypoint2d, **kwargs):
        image, keypoint2d = pad(img, keypoint2d, self.size)
        kwargs.update(keypoint2d=keypoint2d)
        return image, kwargs
