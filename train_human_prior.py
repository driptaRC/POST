...
def main(args: argparse.Namespace):
    ...
    if args.target_val_root is None:
        ...
        val_target_dataset = target_dataset(root=args.target_val_root, split='test', transforms=val_transform,
                                            image_size=image_size, heatmap_size=heatmap_size)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if epoch < args.pretrain_epoch:
            ...
            if epoch == args.pretrain_epoch:
                ...
                train_sf(train_target_iter, student, teacher, prior, con_criterion, 
...
def train_sf(train_target_iter, student, teacher, prior, con_criterion,
    ...
    for i in range(args.iters_per_epoch):
        ...
        with torch.no_grad():
            ...
            y_t_teas = [y_t[j][0] for j in range(len(y_t))] # softmax on w, h
            f_t_teas = [y_t[j][1] for j in range(len(y_t))]
            ...
            for ind in range(x_t_teas[0].size(0)):
                ...
                for _k in range(args.k):
                    ...
                    temp = tF.affine(f_t_teas[_k][ind], 0., translate=[_trans_x/ratio_f, _trans_y/ratio_f], shear=[0., 0.], scale=1.)
                    temp = tF.affine(temp, _angle, translate=[0., 0.], shear=[0., 0.], scale=_scale)
                    temp = tF.affine(temp, 0., translate=[0, 0], shear=[_shear_x, _shear_y], scale=1.) # c, h, w
                    recons_f[_k] = temp # c, h, w
        ...
        with torch.cuda.amp.autocast():
            y_t_stu, f_t_stu = student(x_t_stu, intermediate=True) # softmax on w, h
            ...
            f_t_stu_recon = torch.zeros_like(f_t_stu).cuda() # b, c, h, w
            for ind in range(x_t_stu.size(0)):
                ...
                temp = tF.affine(f_t_stu[ind], 0., translate=[_trans_x/ratio_f, _trans_y/ratio_f], shear=[0., 0.], scale=1.)
                temp = tF.affine(temp, _angle, translate=[0., 0.], shear=[0., 0.], scale=_scale)
                f_t_stu_recon[ind] = tF.affine(temp, 0., translate=[0., 0.], shear=[_shear_x, _shear_y], scale=1.)
            ...
            z_a_norm = (f1 - f1.mean(0)) / f1.std(0) # NxD
            z_b_norm = (f2 - f2.mean(0)) / f2.std(0) # NxD
            z_a_norm = z_a_norm.squeeze()
            z_b_norm = z_b_norm.squeeze()
            # cross-correlation matrix
            c = (z_a_norm.T@ z_b_norm) / f1.shape[0] # DxD
...
if __name__ == '__main__':
    ...
    parser.add_argument('--target_val_root', default=None, help='root path of the target dataset')
    ...
    parser.add_argument("--pretrain", type=str, default=None,
                        help="where restore model parameters from.")
    parser.add_argument('--fix-head', action="store_true",
                        help='freeze regression head')
    parser.add_argument('--fix-upsample', action="store_true",
                        help='freeze upsampling head')
    parser.add_argument("--prior", type=str, default=None,
                        help="where restore prior parameters from.")

    # training parameters
    parser.add_argument('--source-free', action="store_true",
                        help='source free DA')
    ...
    parser.add_argument('--lambda_b', default=1e-4, type=float)
